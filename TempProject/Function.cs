...
namespace TempProject
{
    public class Function
    {
        ...
        DynamoDBContext DynamoContext { get; }
        ...
        public Function()
        {
            ...
            this.Client = new AmazonDynamoDBClient("AKIA3BBTRREXSET5FOMP", "X8Z8USMR8nZX51jkbvCPoG1xPA23rMVF5MzO7h7v", Amazon.RegionEndpoint.USEast2);
        ...
        public async Task FunctionHandler(S3Event input, ILambdaContext context)
        {
            foreach(var record in input.Records)
            {
                ...
                var dic_tag = new Dictionary<string, float>();
                foreach(var label in detectResponses.Labels)
                {
                    if(tags.Count < 10)
                    {
                        ...
                        dic_tag.Add(label.Name, label.Confidence);
                    }
                    else
                    {
                ...
                Photo photo = new Photo("https://ha-sfn-tutorial.s3.us-east-2.amazonaws.com/" + record.S3.Object.Key, dic_tag);
                await DynamoContext.SaveAsync(photo);
                Console.WriteLine("successfully saved to dynamoDB");
            }
            return;
        }
    }
